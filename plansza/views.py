...
def ensure_event_import(request, ident: str):
    try:
        Event.objects.get(facebook_id=int(ident))
    except Event.DoesNotExist:
        event = get_graph(request.user).get_object(id=ident)
        image = get_graph(request.user).get_connections(id=ident, connection_name="picture", type="large")
